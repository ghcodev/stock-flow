...
router.post('/entrada',
  ...
  checkPermission('movimentacoes', 'entrada'),
  ctrl.entrada
);
...
router.post('/saida',
  body('id_lote').isInt({ min: 1 }).withMessage('Lote inválido'),
  body('quantidade').isFloat({ min: 0.001 }).withMessage('Quantidade deve ser maior que zero'),
  checkPermission('movimentacoes', 'saida'),
  ctrl.saida
);
...
router.post('/transferencia',
  ...
  checkPermission('movimentacoes', 'transferencia'),
  ctrl.transferencia
);
...
router.post('/ajuste',
  ...
  checkPermission('movimentacoes', 'ajuste'),
  ctrl.ajuste
);
